...
### v3.10.7：Redis 业务缓存层（R57 审计通过）

- **改的什么**：新增 Redis 业务缓存层，复用既有 `REDIS_URL`（与 `rate_limit` 同源），独立前缀 `blog:cache:`。
  - 新增 `myblog/cache.py`：统一 `cache_get/cache_set/cache_delete/cache_clear_prefix` + `cached` 装饰器；未配置 `REDIS_URL` 时全部静默降级为「无缓存」，连接/读写异常也降级，绝不抛到业务层。
  - `feed_agg.get_circle_feed()`：原进程内 `_CACHE`（每 worker 独立、重启即丢）改为 Redis 共享缓存（key `blog:cache:feed:circle`，TTL 900s），跨 gunicorn worker 一致、重启不丢；后台「诊断助手」强制刷新走 `force=True` 先删 key 再重算。
  - `stats.compute_summary()`：访问统计汇总结果缓存（key `blog:cache:stats:summary`，TTL 120s），大幅削减聚合查询压力。
  - `utils.get_setting()`：站点设置 DB 查询结果缓存（key `blog:cache:setting:<key>`，TTL 300s），读多写少场景省去每次查表。
  - `config.py` 新增 `CACHE_TTL_FEED`/`CACHE_TTL_STATS`/`CACHE_TTL_SETTING`（均可环境变量覆盖）。
- **验证**：`py_compile` 通过；R57 九维审计 **0 遗留**（详见 `myblog/SECURITY_AUDIT.md` R57 轮）；pytest **35 passed**（含新增 `tests/test_cache_layer.py` 4 项：命中/降级/装饰器/集成）。另跑最小 POC 验证「SQLite 下 FastAPI(async) 相比 Flask(sync) 无收益」（async 反而慢 1.25–1.27x），故本次**不迁移 FastAPI**，聚焦 Redis 缓存。
- **⚠️ 部署注意**：**纯后端改动，前端产物无变化**（前端沿用 v3.10.6 的 `_vite_build16`）。部署仅需：① 宝塔/服务器安装并启动 Redis；② 设环境变量 `REDIS_URL=redis://127.0.0.1:6379/0`（同机零网络开销；不设则自动回退「无缓存」，零风险）；③ 覆盖 `myblog-backend.zip` 后「停止 → 启动」gunicorn（restart 不重载）。后台左下角显示 `v3.10.7`。APP_VERSION 升为 v3.10.7。
