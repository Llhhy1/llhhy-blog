...
## 17. v3.18.0 内置业务插件：`page_translate`（全站翻译）— ⚠️ v3.18.3 已移除

> **⚠️ 状态变更（v3.18.3）**：该插件已从仓库移除（`myblog/plugins/page_translate/` 与 `myblog/static/plugins/page_translate/widget.js` 删除，`ENABLED_PLUGINS` 默认值恢复为空），恢复 v3.10.0 的「框架保留、无内置插件」。**本节作为设计参考保留**——日后若要重建全站翻译插件，照此实现即可，框架代码无需改动。
