...
- **全站翻译（v3.18.0 · 插件形态）**：前台左下角浮层「翻译整页 / 显示原文」按钮，一键把**整页**（导航 + 界面文案 + **文章正文** + 动态内容）翻译成另一种语言；引擎**浏览器内置 Translator 优先、站点大模型兜底**（复用「游戏收录 / AI 摘要」的 OpenAI 兼容配置，无需新增密钥）；译文本地缓存、SPA 路由自动续译、随主题自适应。以插件 `page_translate`（`myblog/plugins/page_translate/` + 远程组件 `myblog/static/plugins/page_translate/widget.js`）实现，**核心不再内置中英切换**。
- **插件系统（框架 + 内置 `page_translate`）**：`myblog/plugins/` 可扩展插件框架 + 后台「🧩 插件管理」+ 事件总线与前端槽位；装自写插件只需放目录 + 填 `ENABLED_PLUGINS`；当前内置全站翻译插件 `page_translate`（默认启用）
