...
- **版本确认**：登录后台，左下角显示当前版本（如 v3.1.0），与 [Releases](../../releases) 最新标签比对即可确认部署是否成功。
...
- **修复记录**：第二轮审计修复 Webhook 密钥未从环境变量载入导致恒 403 的缺陷（R1）；第三轮（v2.3.0）修复邮件注入 / 文件句柄泄漏 / 邮箱枚举；最新一轮（v3.1.0，R8）新增登录审计日志 + 30 天保留 + 打包下载，并修复手机端汉堡菜单不随深色模式切换的问题。
