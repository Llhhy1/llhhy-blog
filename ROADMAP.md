...
- **C3 邮件订阅 / Newsletter**：侧边栏订阅框 + `/api/subscribe`（去重 + 限流）✅（**尚未实现群发邮件**，见「后续可做」）

### 模块 D：运营与分发 ✅
- **D1 一键分享卡片（OG 标签）** ✅
- **D2 微信 / Telegram 新文推送**：`notify.py`（Telegram + 企业微信）✅
- **D3 Webhook 自动部署**：`/api/webhook/deploy`（HMAC 校验）✅
...
- **C3 群发邮件**：订阅者已可收集邮箱，但尚未实现「新文章发布 → 邮件群发」；需配置 SMTP（宝塔可装 postfix 或使用第三方邮件服务）。
- **A4 @ 通知**：评论/动态 @ 某人时的站内通知（当前仅显示 @ 文本）。
- **D3 真正自动部署**：当前 Webhook 接口只做密钥校验返回 `{"ok":true}`，需配合宝塔 Webhook 脚本才能实现「GitHub push → 自动拉取重启」。
- **评论审核流**：当前评论直接可见（有 IP 属地可追溯），可加「先审后发」开关。
