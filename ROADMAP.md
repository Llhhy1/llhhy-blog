...
- **双前端结构 → 已决策「退役公共 SSR」，2026-09-20 落地 v3.18.6**：生产 Nginx 只反代 `/api/` `/admin` `/static/` `/mcp*` 与 feed/sitemap/robots，其余走 `try_files … /index.html`，则 `routes.py` 的 8 个公共 SSR 页面永远收不到流量。**处置**：这 8 个路由退役为 410 Gone（endpoint 名保留，避免 `base.html` 的 `url_for` 让登录页 `BuildError`），7 个死模板已删除。→ 从此**只有一套渲染器（Vue SPA）**。
  - **仍在的 SEO 缺口（未做）**：`/post/*` 有爬虫专用通道（Nginx 把匹配爬虫 UA 改写为 `/api/og/post/<slug>`，服务端渲染 OG meta）——**但只有 OG、没有 JSON-LD**（JSON-LD 原本只在已退役的 SSR 文章页，现已彻底没有）；`/`、`/archive`、`/category/*`、`/tag/*`、`/about`、`/links` 仍是 SPA 空壳、无任何服务端 meta。可选解法：① 给这几类路径加同款的 `og_*` 服务端 meta 路由 + Nginx bot 改写；② 引入构建期预渲染（vite-plugin-prerender / SSG）；③ 保持现状（接受百度/搜狗/微信抓取拿不到这些页的 meta）。
