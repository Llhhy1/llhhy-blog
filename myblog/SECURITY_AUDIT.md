...
## 第七十一轮 R71（v3.17.1 · 导航 CSS 选择器被 autoescape 转义的修复）

**范围**：`myblog/app.py` 的 `theme_nav_css` 选择器由 `html:not([data-theme="dark"])` 改为 `html:not([data-theme=dark])`（去引号）；补回同批次被波及的 `theme_css` 定义；新增 `tests/test_theme_nav_css.py` 回归测试。

| 编号 | 维度 | 审计点 | 结论 |
|---|---|---|---|
| R71-1 | 缺陷性质 | 属**前端渲染缺陷**（选择器被 HTML 转义失效），非安全漏洞：不涉及越权、注入、数据泄露或信任边界变化。 | ✅ 非安全面 |
| R71-2 | 注入/XSS | `theme_nav_css` 拼接内容仍仅为内部常量（`#hex` 由 `nav_style` 固定映射产生，非用户输入），未引入新的可注入点；改后输出无引号亦不改变转义策略（未使用 `\|safe`）。 | ✅ 无新增注入面 |
| R71-3 | 回归 | 新增 2 条用例（选择器可用 + 变量仍在）通过；全量 pytest 84 passed。 | ✅ 无回归 |

**R71 结论**：**0 遗留**。发版前 `APP_VERSION` 改为 `3.17.1`（与 Release tag 一致）。

---

## 第七十二轮 R72（v3.17.2 · 后台移动端修复 + 打印样式 + 成就徽章）

**范围**：`myblog/static/admin.css`（新增 ≤760px 移动端规则组，**纯 CSS**）；`vue-frontend/src/styles/global.css`（新增 `@media print`）；`myblog/api/review.py` 新增 `GET /api/milestones`（公开只读聚合）；`AnnualView.vue` 徽章区。

| 编号 | 维度 | 审计点 | 结论 |
|---|---|---|---|
| R72-1 | 新增接口越权/PII | `GET /api/milestones` 为公开只读，仅返回**聚合计数**（发文数/阅读总量/评论数/连续天数/开博天数）与徽章解锁布尔值，**不含 IP、邮箱、用户名、文章内容**。 | ✅ 无敏感数据 |
| R72-2 | 注入/XSS | 前端徽章渲染全部 `{{ }}` 插值（Vue 自动转义），徽章文案为服务端常量，无用户输入参与；接口无 SQL 拼接（纯 ORM 聚合）。 | ✅ 无注入面 |
| R72-3 | 资源/DoS | 该接口为全表聚合（`_visible_posts().all()` + `Comment.count()`），与既有 `/api/review/annual` 同源同量级；无缓存但为只读轻查询，无循环/递归放大；连续天数计算以日期集合做 O(n) 扫描。 | ✅ 有界 |
| R72-4 | 写操作/存储 | **无任何写操作**；徽章状态为实时计算，未新增表、未新增列、未写 Setting —— 符合「能不改表就不改」纪律，**零迁移**。 | ✅ 零迁移 |
| R72-5 | CSS 安全性 | 移动端规则与打印样式均为静态样式表修改，不引入 `url()`/`expression()`/远程资源；`!important` 仅用于覆盖既有版式，不涉及脚本。 | ✅ 无新增风险 |
