...
### 模块 E：插件系统（全栈，v3.9.0 落地）✅
- **E1 插件加载框架**：`myblog/plugins/` 动态加载 `register(app, cfg)`，失败隔离（单插件崩不拖垮博客）✅
...
- **更新链完整性校验 fail-open → ✅ 已闭环，2026-09-21 落地 v3.18.7**：原问题是 `sha256.txt` 与 zip 走同一 `gh_fetch` 通道（且自动降级到 `ghfast.top` / `gh-proxy.com` 等第三方镜像），4 处 `⚠️ + return 0` 任一处即可静默关掉校验，HMAC 还只在首行以 `HMAC ` 开头时才校验（删首行即关闭）——「双源互证」与 zip 同源自指。**处置**：引入 **Ed25519 分离签名**（`sha256.txt.sig`），**公钥内置 `update.sh`**（clone 即用、可被 `RELEASE_PUBKEY` 覆盖），7 处 fail-open 全转 fail-closed，**取消自动第三方镜像**（只认显式 `GH_MIRROR`），新增版本单调性与两个显式逃生舱（`ALLOW_UNSIGNED` / `ALLOW_DOWNGRADE`，默认关闭）。`deploy.sh` 从独立副本收敛为 update.sh 的薄封装（审计面减半）。安全审计 **R87**；反向实测 20/20 + 125 passed。
- **双前端结构 → 已决策「退役公共 SSR」，2026-09-20 落地 v3.18.6**：生产 Nginx 只反代 `/api/` `/admin` `/static/` `/mcp*` 与 feed/sitemap/robots，其余走 `try_files … /index.html`，则 `routes.py` 的 8 个公共 SSR 页面永远收不到流量。**处置**：这 8 个路由退役为 410 Gone（endpoint 名保留，避免 `base.html` 的 `url_for` 让登录页 `BuildError`），7 个死模板已删除。→ 从此**只有一套渲染器（Vue SPA）**。
  - ~~**仍在的 SEO 缺口（未做）**：`/post/*` 有爬虫专用通道…**但只有 OG、没有 JSON-LD**…~~ → **2026-09-21 落地 v3.18.9（阶段 1 完成）**：`/post/*` 的爬虫通道**已修复并补齐**——① 三出口语义（不可见→404+noindex / 经通道+抓取方→200 壳页+**index,follow** / 其余→302 回 SPA），修掉了「百度进来后被告知 noindex」的自废收录；② 壳页**带正文**（复用 `content_html` 缓存列）+ **`BlogPosting`/`BreadcrumbList` JSON-LD**（原 SSR 文章页退役后彻底缺失的正是这块）；③ 微信首次真正可被分流（`detect_bot()` 认不出 `MicroMessenger`，改用独立 UA 表，并加**真人信号双层否决**防 QQ 内置浏览器真人误伤）；④ 站点地址收敛 `site_base()`，关闭 Host 注入遗留项；⑤ Nginx 配置**原文入库** `deploy_guide.md`「第 4b 步」。安全审计 **R89**；142 passed + 变异测试验证断言有效性。
  - **仍未做的 SEO 缺口**：`/`、`/archive`、`/category/*`、`/tag/*`、`/about`、`/links` 仍是 SPA 空壳、无任何服务端 meta。可选解法：① 给这几类路径加同款的 `og_*` 服务端 meta 路由 + Nginx bot 改写；② 引入构建期预渲染（vite-plugin-prerender / SSG）；③ 保持现状。~~另：百度主动推送 / IndexNow 提交（SEO 实施清单**阶段 2**「收录控制台」）尚未开始。~~ → **2026-09-21 落地 v3.19.0（阶段 2 完成）**：后台「🔍 收录」控制台——新表 `SeoSubmission`（每篇 × 每引擎一行，明确不用 `Setting` KV）、`seo_push.py` 推送引擎（百度主动推送 + Bing/IndexNow；唯一 http 出站目标做**精确 host 白名单**；批量一次请求；`quota` 独立成档）、凭据 `encrypt_secret` 密文存储 + 掩码回显、推送**全异步**（daemon 线程 + 页面轮询）、不可见文章拒绝推送、sitemap/robots 只读预览复用既有视图、GSC/Bing/百度站长入口。**Google 不做自动推送**（无通用 API）。安全审计 **R90**；171 passed（+29 条）。
  - **推荐做法（若日后要提升收录）**：优先配 **IndexNow key**（配额宽松、一条命令验证），再配百度 token；两者都需要 `site_url` 已配置（否则推送 URL 无域名，页面会红字提示）。GSC 仍建议手工提交 sitemap 并观察覆盖率。
...
#### 5.9.1 处置状态（2026-09-23 · v3.20.0 已逐项处理）

> 下面 A–E 是**审计原文记录**（保留不改，便于回溯）。实际处置见本小节；**详细依据与实测数据在 `SECURITY_AUDIT.md` R92**。
>
> 本轮的方法论是「**先实测再动手**」，因此有 3 项结论与审计描述不同 —— 不盲从清单是本轮的主要收获。

| 项 | 状态 | 说明 |
| --- | --- | --- |
| **D. CI / 工程化** | ✅ **已做** | `pyproject.toml`（ruff 阻断集）+ `tools/lint_debt.py` **债务棘轮**（只减不增）+ CI `lint` job（含 i18n、发布公钥一致性、**pip-audit CVE 扫描**）+ `dependabot.yml` + `npm ci` + **覆盖率门禁（49% 基线，`--fail-under=45`）** |
| **C. 可观测性**（部分） | ✅ **已做样板** | 新增 **`GET /api/health`**（原先零探活端点）；`notify.py` / `mail_notify.py` 的 `print` 与静默 `except: pass` → `logger.warning(exc_info=True)`。**其余 91 处 `except: pass` 未批量改** —— 涉及「记日志还是静默」的语义决策，已纳入棘轮止损 |
| **A. 性能 · 同步外网** | ✅ **已做** | `notify.py` 改为后台线程（原最坏阻塞 12s）。**顺带更正**：我原以为「传 ORM 对象进线程会炸」，实测证伪 —— 见 R92 §92.3 |
...
| **E. 仓库卫生** | ⏳ **未做** | 文档归档到 `docs/`、版本号收敛、`create_app()` 拆分、真 Alembic 基线。本轮**有意不碰**（改动面大、与功能批次混在一起会稀释可验证性） |
| **A. 其余性能项** | ⏳ **未做** | `eager loading`（仍 **0 处**）、`.all()` 107 处、列表 N+1、`/static/` 的 gzip/expires。**建议下批**：先量真实耗时再改 |
| **B. 其余 a11y** | ⏳ **未做**（3 项） | 4 组 token 对比度 < 4.5:1 与 `derive_dark` 无 WCAG 校验｜表单 placeholder-only｜后台 `data-label`(**2/50**)｜`tokens.css` 两份且**已漂移**（2004 vs 2291 字符，需人工判断哪份为准再合并） |
...
| **SEO 自动推送（§5.4）** | ✅ **已做（替代方案）** | 原「ping 百度 / Google / Bing」端点实测不可用（Bing 410、Google 超时）→ 改为「发布即自动 `enqueue`」（`maybe_auto_push`），**默认关闭**、只推可见文章、复用限流去重；新增可选环境变量 `SEO_AUTO_PUSH` |

#### 5.9.2 审计原文（记录，不再作为待办清单）
...
## 48. v3.8.6：博客圈自诊断 + 系列热门标签 + 文档页导航

- **① 博客圈自诊断（可观测性）**：`feed_agg.py` 新增 `_LAST_DIAG` 诊断收集；`/api/feed/circle` 响应附 `debug` 块（友链总数 / 已填 RSS 数 / feedparser 是否安装 / 成功抓取 / 跳过数 / 每条友链原因）；`SquareView.vue` 博客圈为空时显示「聚合诊断」面板 + 「强制刷新」按钮。根因：用户反馈博客圈异常却看不到原因，现场无迹可查；现前端直显，无需登服务器看 gunicorn.log。
- **② 系列详情页热门标签**：`SeriesDetailView.vue` 用 `computed` 统计该系列文章标签频次（取前 20）以标签云展示「🔥 本系列热门标签」，点击跳 `/tag/:slug`。纯前端计算，复用 `/api/series/:slug` 已返回的 `posts.tags`，无后端改动、无新接口。
...
## 68. v3.15.0：标签治理 + 分享卡片 + 游戏平台（R65 审计 0 遗留）
...
## 70. v3.17.0：深色修复 + 动效/无障碍 + AVIF + 年度回顾 + AI 摘要（R70 审计）

- 修复三项（含用户报障）：深色模式顶部白条（前后台 SSR 同源：`nav_style` 写死 `--nav-bg` 且优先级压过 dark）、主题包换肤未生效（token key 下划线 vs CSS 短横线）、后台表格手机端无法浏览全文（窄屏卡片化）。
...
- v3.17.13 **UI 两项**：后台「数据备份」页移动端重构（概览 7 卡 + 备份列表窄屏转卡片，数据由 `backup.py` 新增统计工具算、零新表）；前台导航 13 项统一加 `.nav-emoji` 图标，抽屉与桌面顶栏同步，风格对齐后台侧栏。
  - **后续可做（审查未采纳项）**：① 超长文件拆分 —— ✅ **v3.18.1 已完成**：`admin/posts.py`（852 行 / 26 路由 → 5 个领域模块）、`utils.py`（784 行 → `utils/` 8 子模块包）；`app.py`（783 行，应用工厂 + 自动迁移，聚合度高且非「26 路由挤一文件」之患）评估后**不拆**（收益低于风险）。
...
- v3.18.0 **全站翻译插件化**：移除核心中英切换（`store.js` 的 `I18N`/`t()`/`setLang`/`initLang`/`state.lang` + `App.vue` 24 处 `t()` 与两个语言按钮 + `global.css` 的 `.lang-toggle`）；新增内置插件 `page_translate`（远程组件 `myblog/static/plugins/page_translate/widget.js`，浮层整页翻译含正文，浏览器 Translator 优先 / 站点 LLM 兜底，CSRF + 双层限流 + 目标白名单 + 长度上限，零落库）；`ENABLED_PLUGINS` 默认改为 `page_translate`。验证 113 passed。（⚠️ **v3.18.3 已移除该插件**，`ENABLED_PLUGINS` 恢复为空）
...
- v3.18.6 **退役公共 SSR（架构决策落地）**：`routes.py` 的 8 个页面级 SSR 路由（`/`、`/post/<slug>`、`/category/<slug>`、`/tag/<slug>`、`/search`、`/about`、`/links`、`/archive`）**退役为 410 Gone**，删除 7 个死模板（`myblog/templates/` 顶层只剩认证 + 错误页 8 个文件）。保留 endpoint 名（`base.html` 的 `url_for` 依赖它们，删路由会让登录页 `BuildError`）；不做 302（与 SPA 同名会自环）。`/login` `/register` `/logout` 仍为 SSR，两个 POST 交互入口保留。安全审计 **R86**（攻击面净减少：顺带消掉 SSR 侧未加固的 `/search` 全表 LIKE 与文章页评论全量加载）。验证 **119 passed**（+7 条回归）。**至此只有一套渲染器**。
- v3.18.7 **更新链 fail-closed + Ed25519 发布物签名**（审计判定的全仓最高风险项闭环）：`package.py` 打包后对 `sha256.txt` 做 Ed25519 分离签名 → 新资产 `sha256.txt.sig`（首次运行**自动生成**密钥对到 `~/.workbuddy/llhhy_release_key`，支持 `--gen-key` / `--no-sign` / `RELEASE_SIGNING_KEY`）；`update.sh`/`deploy.sh` **内置验签公钥**（`BUILTIN_RELEASE_PUBKEY`，可被 `RELEASE_PUBKEY` 覆盖）→ 任何人 clone 部署开箱即可获得签名保护。7 处 fail-open 全转 fail-closed；**取消自动第三方镜像兜底**；新增版本单调性（同版本收工 / 默认拒降级）与 `ALLOW_UNSIGNED=1` 显式逃生舱；`deploy.sh` 从 529 行独立副本收敛为 45 行薄封装。`verify_package_checksums.py` 增加**双源互证 ③**（用 update.sh 读出的同一公钥验签）。安全审计 **R87**。验证 **125 passed**（+6 条契约测试）+ **反向实测 20/20**（直接跑 update.sh 里真实的内联验签代码）。
- v3.18.8 **修复 v3.18.7 上线实测发现的验签顺序缺陷**：`$WORK`（`/tmp/llhhy_update`）跨轮复用且只清解压目录 → 上一轮遗留的 `sha256.txt` 被拿来与本轮新的 `sha256.txt.sig` 配对比对，**必然验签失败**（v3.18.7 首次上线即 BAD；**站点未受影响**——fail-closed 在覆盖代码前就拒绝了安装，顺带实证了该保护有效）。修复：验签函数改为自包含（先清旧清单、再自行下载两者），`$WORK` 准备阶段显式清清单与包，新增契约测试钉死。安全审计 **R88**。验证 **126 passed** + 服务器端到端实跑全绿。教训：**跨轮复用的工作目录 + 校验文件 = 隐性状态泄漏**。
