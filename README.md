...
- **运维**：访问统计（区域 / 热读 / 热搜 / 时段）、数据备份与异地容灾（本地 / OSS / SCP / WebDAV）、后台一键在线更新、全站健康体检（11 维）
...
- **Redis 业务缓存层（v3.10.7）**：复用既有 `REDIS_URL` 新增博客圈 RSS 聚合 / 访问统计汇总 / 站点设置三类读多写少热点的 Redis 缓存（前缀 `blog:cache:`）；未配置 Redis 自动回退「无缓存」，零风险
